...
import (
	...
	"github.com/LucasAntonioC-137/crud-go/src/tests/connection"
...
func TestMain(m *testing.M) {
    os.Setenv("MONGODB_USER_DB", "test_user")
    os.Setenv("MONGODB_URL", "mongodb://mongo:27017") // URL do container

    var closeConn func()
    Database, closeConn = connection.OpenConnection()
    defer closeConn()
...
func TestFindUserByID(t *testing.T) {
	...
	t.Run("user_found_with_this_ID", func(t *testing.T) {
		...
		_, err := Database.
			Collection("test_user").
			InsertOne(context.Background(), bson.M{"_id": id, "name": t.Name(), "email": "test@lucas.com"})
		if err != nil {
			t.Fatal(err)
			return
		}
